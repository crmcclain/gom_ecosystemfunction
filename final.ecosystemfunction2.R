...
####load and process data#####
  ...
  logbytaxa<- data.frame(read.csv("woodfall_logbytaxa_ef.csv", header=TRUE, stringsAsFactors=FALSE))
  logsummary <-  data.frame(read.csv("woodfall_logs_ef.csv", header=TRUE, stringsAsFactors=FALSE))
  NCOL <- ncol(logbytaxa)
  ...
  logdiversity_drop <- logdiversity %>%
    ...
    filter(!grepl("L-B", log_id)) #dropping other experimental wood falls not part of original analysis
   
  
####bivalve spatial data####
  ...
  pdf(file="figure.pdf",width=8, height=6)
  ...
  #new plot for supplemental
  
  ggplot(data=logposition, aes(x=Z_.mm., y=Y_.mm., color=Xylo_Group))+
    geom_point(cex=3)+
    xylo_pallette+
    xylo_pallette2+
    theme_bw()+
    labs(y="Proxmity to End, Y distance (mm)", x="Surface Preference, Z distance (mm)")
...
####phylogenetic diversity####
      ...
      logdiversity_drop <- left_join(logdiversity_drop, comm.pd, by=c("log_id"="wood_id")) %>%
        select(Abundance, S, H, Simp, log_id, site,log10Mass:hardness, PD, wood.type, log10Mass, collection_set)
...
#####Figure alpha diversity #####
  ...
  pdf(file="figure.pdf",width=11.5, height=8)
  par(mar=c(5,3,2,2)+0.1) #removes space from around edges of pdf
  grid.arrange(xylo_A, xylo_S, xylo_J, xylo_PD)
  dev.off()    
...
###simple ecosystem function models####
  
logdiversity_drop_noout <- logdiversity_drop %>%
    filter(log_id != "L-Y-31")
    
    #abundance reduced model and comparisons
    glm_A <- glm(log10consumed ~ xylo_Abundance,data=logdiversity_drop)
    summary(glm_A)
    gamm_A <- gamm(log10consumed ~ s(xylo_Abundance),data=logdiversity_drop)
    summary(gamm_A)
    AIC(glm_A, gamm_A)
...
#####overall comparison of models####
 ...
 logdiversity_drop2<-logdiversity_drop %>%
   filter(xylo_Abundance>0) %>%
   mutate(sd_sz=coalesce(sd_sz,0)) %>%
   select(log10consumed, xylo_Abundance, xylo_S, hardness, PD, site, collection_set,) %>%
   na.omit()

    model_all <-  gls(log10consumed ~ xylo_Abundance + xylo_S + xylo_S*hardness +PD + site +collection_set,
                     data=logdiversity_drop2, na.action='na.fail')
   
   summary(model_all)
   
   dr_model_all <- dredge(model_all,extra = c("R^2", F = function(x)
     summary(x)$fstatistic[[1]]))
   
   dr_model_all_modelavg<-model.avg(get.models(dr_model_all, subset= delta <2))
   summary(dr_model_all_modelavg)
   
  
   
   ##
   bestmodel2 <- glm(log10consumed ~ xylo_Abundance + PD + site +collection_set,
                     data=logdiversity_drop2, na.action='na.fail')
   
   bestmodel1 <- glm(log10consumed ~ xylo_Abundance + site +collection_set,
                     data=logdiversity_drop2, na.action='na.fail')
 
   r2_nagelkerke(bestmodel2)
   r2_nagelkerke(bestmodel1)
     ...
     model_sum <- data.frame(summary(bestmodel2)$coefficients)
     ETA <- data.frame(epsilon_squared(aov(bestmodel2))[2])
     model_sum$Epsilon2_partial[1] <- NA
     model_sum$Epsilon2_partial[2:6] <- ETA$Epsilon2_partial
     
     #row.names(model_sum) <- c("Intercept", "Abundance", "Richness","Phylogenetic Diveristy")
...
#####basic stats for paper#####
    ...
    glm_S_A_hard_PD_sz <-glm(log10consumed ~ xylo_Abundance + xylo_S + xylo_S*hardness + PD + site,data=logdiversity_drop)
...
####10 logs analysis####
    ...
    #10 log alt models
    glm_A_TNW_sz_10 <-glm(log10consumed ~ xylo_Abundance+TNW+sd_sz,data=tenlogs)
    summary(glm_A_TNW_sz_10)
    
    glm_A_TNW_10 <-glm(log10consumed ~ xylo_Abundance+TNW,data=tenlogs)
    summary(glm_A_TNW_10)
    AIC(glm_A_TNW_sz_10, glm_A_TNW_10,glm_A_10)
    ...
    cor.test(tenlogs$xylo_Abundance, tenlogs$TNW, method=c("pearson"))
  
    cor.test(logsummary$weight_final, logsummary$wood.mass.final, method=c("pearson"))
    plot(logdiversity_drop$log10Mass, logdiversity_drop$log10consumed)
